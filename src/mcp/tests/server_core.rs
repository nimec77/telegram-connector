...
fn tools_list_carries_cache_hints_and_stable_order() {
    ...
    // Then: All 13 tools are present, with SEP-2549 cache hints attached
...
fn tools_list_hints_gated_on_protocol_version() {
    ...
    assert_eq!(current.tools.len(), 13);
    ...
    assert_eq!(legacy.tools.len(), 13); // tool list itself is unaffected
