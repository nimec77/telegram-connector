...
pub trait TelegramClientTrait: Send + Sync {
    ...
    /// Issues the same RPC as grammers' `get_messages_by_id`, but keeps the
    /// response envelope so forward attribution resolves.
    ...
    /// (an id vector, response envelope preserved).
