...
| 31 | Polish (work-order D4–D8) | ✅ Complete | 560 | `get_message_media` passes an already-fitting JPEG source through byte-identical instead of re-encoding it, eliminating quality loss and a measured +29% size inflation (D4); `check_mcp_status` gains a nested `rate_limiter` budget block (`tokens`/`capacity`/`refill_per_sec`/`costs`), with `rate_limiter_tokens` kept as a deprecated alias until v0.18 and rate-limit rejections now stating requested-vs-available tokens (D5); `get_last_responses` gains `include_binary` (default `false`) to stub image payloads on replay unless explicitly requested (D6); tool-level error prefixes (`parse_channel_id`/`parse_message_id`) apply exactly once instead of stacking with the inner `invalid input:` prefix (D7); malformed channel usernames are rejected locally before spending a resolve RPC, surfacing the same clean not-found error (D8) |
| 32 | Surface (work-order A1, A3+A6, A5, A7, D5 alias removal) | ✅ Complete | 601 | Three new tools bring the total to 15: `get_messages_batch` (13th, ≤50 ids in one RPC, per-id `missing` instead of a failed batch, byte-budget overflow reported in `omitted_ids`, A1); `resolve_channels` (14th, ≤20 identifiers — numeric id/`@username`/exact subscribed title — one dialog walk + at most one `resolve_username` RPC per unmatched username, per-identifier failures are data not call errors, A7); `get_channel_stats` (15th, album-collapsed posting-rate/engagement sample, default 7d/max 30d/500-message scan cap, `posts_per_day` divides by the sampled span floored at 1h, `sample.complete` flags a cap-truncated sweep, A5). `channel_ids` fan-out (1-20 channels, bounded concurrency 4, `acquire(N)` rate cost) added to both `get_recent_messages` and `search_messages` — merged newest-first, truncated to `limit`, partial failures in `channel_errors`, multi-channel `format: "compact"` hoists into a `channels` map keyed by channel id while messages keep their own `channel_id` (A3/A6); restored work-order §1.3 username acceptance on `search_messages.channel_id` via `resolve_channel_identity` (regressed since numeric-only `SearchParams.channel_id` landed); scheduled removal of the `rate_limiter_tokens` deprecated alias — its one-release grace period (v0.17) ended (D5) |
...
| 34 | Converter parity (work order A) | ✅ Complete | 656 | `forwarded_from` enrichment reaches `get_message_by_link` / `get_messages_batch` via a raw envelope-preserving `getMessages` (`raw_pager::fetch_messages_by_id`); `get_channel_stats` moved to `RawHistoryPager`; `convert_message` + `EntityLookup::insert_peer` deleted, `EntityLookup::empty` gated `#[cfg(test)]` with a non-derived body (the `#[derive(Default)]` hole a review caught would have left `EntityLookup::default()` as an ungated equivalent), making envelope-less conversion unrepresentable; `require_found_raw` twin added for the two raw-TL callers, `download_message_media`/`transcribe_audio` unchanged. Adds `document_info`, `poll_info` (per-option `voters`, never fabricating an undisclosed `0`), and `audio_info.title`/`performer` — all zero-call. Live acceptance run 2026-08-14 and passed: `forwarded_from` for `1912881684/298716` byte-identical (`"Pavel Zloi"`) across `get_message_by_link`/`get_messages_batch`/`get_recent_messages`; `document_info` populated for `2246801752/198`; `poll_info` verified against a real poll (`1734769861/46611` — 8 options with real per-option voter counts, `total_voters` 373, identical through the raw-envelope batch path and `get_recent_messages`) — see docs/memory.md |

| 35 | Global search latency (work order B) | ✅ Complete | 676 | Unscoped `search_messages` no longer walks Telegram's entire global index client-side: `messages.SearchGlobal`'s `min_date`/`max_date` (previously hardcoded to `0`) are now sent, moving the time-window filter server-side — measured live, limit 20: `document`/24h 44.86s→0.449s, `video_note`/24h 12.93s→0.411s, `voice`/72h 8.07s→0.468s, `url`/24h 0.46s→0.500s (already-fast path unaffected); client-side window checks retained as defense in depth, result sets unchanged. New `[search] deadline_seconds` (default 20, validated `1..=3600` — the upper bound guards `SearchBudget`'s `Instant` arithmetic from overflow) bounds a search's accumulation loop; on expiry the search returns what it gathered with `query_metadata.timed_out`/`partial` set, never an error. `query_metadata.pages_fetched`/`messages_scanned` (always present, on both `search_messages` and `get_recent_messages`) report result pages fetched and raw messages walked, summed across a `channel_ids` fan-out while `timed_out`/`partial` OR across it; `get_recent_messages` never sets the deadline flags (the deadline is scoped to search). Deadline truncation deliberately leaves `has_more`/`next_cursor` unset — expiry proves nothing about what lies beyond the page, and global search has no per-channel cursor to resume from regardless. Debug-level tracing (`search_global` span + per-page/summary events) added for diagnosability. Live acceptance run confirms the fix on a real session, and a follow-up run (2026-08-14) exercised the deadline-expiry path itself live: with `deadline_seconds = 1`, a channel-scoped search returned a graceful partial in 1.053 s with `timed_out`/`partial` set, 1 page / 100 scanned, no cursor and no error (see docs/memory.md) |

| 36 | Media throughput (work order C) | ✅ Complete | 709 | New 16th tool `get_messages_media_batch`: up to 10 message ids from one channel returned as `[image, metadata, image, metadata, …, summary]` content blocks in request order; resolves the channel once and issues one `get_messages_by_id` for all ids, then downloads with bounded concurrency (4, the existing `channel_ids` fan-out constant) — one channel resolution and one fetch round trip for the whole batch instead of one of each per image, which matters because a numeric `channel_id` resolution is a full uncached dialog walk. Per-id failures (`not_found`, `no_visual_media`, `payload_cap_reached`, `download_failed: <detail>`) are reported in the summary's `failed` array and never fail the batch. New `[limits] media_batch_max_total_bytes` (default 8 MiB) caps the batch's total base64 payload as sent to the client; images are downscaled progressively to fit, and ids that still don't fit are reported as `payload_cap_reached`. `[rate_limiting]` retuned: `max_tokens` 50 → 60, `media_download_cost` 5 → 3, `refill_rate` unchanged at 2.0 — 20 images in a burst then one per 1.5 s, up from 10 then one per 2.5 s. A batch acquires `media_download_cost × requested_ids` up front and refunds ids that produced no image, so the net charge is per image returned; a whole-call failure (unresolvable channel, fetch RPC error, FLOOD_WAIT) refunds all but 1 token, matching what `get_messages_batch` charges for the same resolve+fetch shape of work. `check_mcp_status` gains an additive `media` block (`batch_max_ids`, `max_total_bytes`, `per_image_max_bytes`, `default_max_dimension`, `max_dimension_limit`). Task 10 (live acceptance) ran and passed against the deployed session: 10 photos from one channel cost 13.10 s sequentially vs 2.99 s batched (4.4×), and 10.34 s vs 1.44 s (7.2×) on a second channel, each arm in a fresh process after a cooldown; the payload cap was confirmed live (5 returned totalling 390 228 bytes under a 400 000-byte cap, 5 ids `payload_cap_reached` in request order). A first attempt that ran the two arms back to back reported the batch as *slower* (19.59 s) because Telegram throttled the batch's `iter_dialogs` walk to ~18 s after the preceding download burst — see docs/memory.md. At acceptance time the deployed `config.toml` pinned `max_tokens = 30` and `refill_rate = 1.0` (so the `max_tokens` default change did not reach that deployment and one full 10-image batch cost the entire bucket); the deployment was refreshed after the v0.22.0 release and now runs the shipped defaults (60 / 2.0). Plan: `docs/superpowers/plans/2026-08-13-media-throughput.md`, design: `docs/superpowers/specs/2026-08-13-media-throughput-design.md` |

| 37 | Media batch review fixes | ✅ Complete | 722 | Ten findings from a code review of Phase 36's `get_messages_media_batch` closed across eleven tasks on `fix/media-batch-review`: cap exhaustion gets its own `Error::PayloadCapExceeded` instead of sharing `Error::DownloadFailed` with genuine decode failures, so a batch id that downloaded but could not be shrunk under its remaining allowance now reports `payload_cap_reached` instead of the unretryable `download_failed` (Tasks 1-2, findings 1/10); explicit `returned` success counter replaces `content.len() / 2` (Task 3, finding 9); the batch refund saturates like the charge it reverses (Task 4, finding 2); rate-limit costs above `max_tokens` are rejected at config-load time (Task 5, finding 2); image encoding moved off the async worker onto a blocking thread, with the batch loop staying sequential and in request order so payload-budget allocation is unchanged (Task 6, finding 3); both module-layering inversions removed — `src/telegram/` no longer imports `crate::mcp` for its download concurrency constant, and `config.rs` no longer imports a validation bound from the MCP layer (Tasks 7 and 9, findings 4/5); grammers' one-slot-per-id contract is asserted via `debug_assert_eq!` instead of padded for (Task 8, finding 8); `McpServer::new` builds its six defaults from `config::defaults` instead of hand-copied literals, guarded by a desync test (Task 10, finding 7); id dedupe/validation shared between the two batch tools (Task 11, finding 6). Task 12 (docs + release): README, the tool description, and the `MediaBatchFailure.reason` JSON-schema description all gain the `internal_error: <detail>` token and document that `payload_cap_reached` covers both total-budget exhaustion and an image that downloaded but could not be shrunk to fit; CHANGELOG records the user-visible `get_message_media` error-text change (`Error::PayloadCapExceeded`'s Display text no longer carries `Error::DownloadFailed`'s `"media download failed: "` prefix) plus a restored `[0.22.0]` heading a prior docs commit had dropped; two stale comments corrected (`RateLimitConfig::validate`'s overflow claim, an off-by-one grammers line citation). Released as **v0.22.1**. Plan: `docs/superpowers/plans/2026-08-14-media-batch-review-fixes.md`, design: `docs/superpowers/specs/2026-08-14-media-batch-review-fixes-design.md` |

| 38 | Audit stage 1: correctness + dead code | ✅ Complete | 723 | Full-codebase audit (spec: `docs/superpowers/specs/2026-08-15-project-audit.md` — 4 staged work packages; coverage baseline 75.1% lines via `cargo llvm-cov`). Stage 1 (plan: `docs/superpowers/plans/2026-08-15-audit-stage1-correctness.md`): env-mutating config tests self-serialize via `ENV_LOCK` so plain `cargo test` is race-free (the `test-config` serial recipe is retired); `redact_phone` char-aware + used by `interactive_auth` (was a panic path and a redaction bypass); `wire_message_id` rejects ids beyond Telegram's i32 wire range at the three single-message tool sites (previously wrapped silently); dead code removed (`TelegramClient::sign_in`, `parse_optional_channel_id`, `matches_media_filter`, placeholder auth test; `overflowed` gated `#[cfg(test)]`). Stages 2-4 (module/test splits, dedup refactors, ops-layer coverage) specced, not started |
| 39 | Audit stage 2: module & test splits | ✅ Complete | 705 | Stage 2 of the full-codebase audit (plan: `docs/superpowers/plans/2026-08-15-audit-stage2-splits.md`, spec: stage 2 of `docs/superpowers/specs/2026-08-15-project-audit.md`). `client/raw_pager.rs` split into `raw_page.rs` (envelope interpretation) + `raw_fetch.rs` (by-id fetch) + the pagers themselves; test extraction into `client/tests/` (`raw_pager_tests.rs`/`raw_page_tests.rs`/`raw_fetch_tests.rs`/`channels_tests.rs`) and `telegram/tests/message_tests.rs`; `telegram/converters` tests split three ways (`converters_av_tests.rs`/`converters_doc_poll_tests.rs`/`converters_thumb_forward_tests.rs`); `mcp/tests` search/history/media_batch each split into three files (`search_core`/`search_dates`/`search_shaping`; `history_core`/`history_dates`/`history_paging`; `media_batch_core`/`media_batch_budget`/`media_batch_fixtures`); `config/tests.rs` split four ways (`defaults_tests.rs`/`env_tests.rs`/`load_tests.rs`/`validation_tests.rs`) with a new `EnvGuard` RAII drop-guard that restores every touched env var on drop, even on panic (closes a stage-1 review finding). 19 mock-only `TelegramClient` tests pruned from `client_tests.rs` (the 6 `username_to_resolve` tests were kept — they exercise real resolution logic, not just the mock). Fixture consolidation into `test_helpers.rs` (`raw_tl_message`, `raw_tl_messages_slice`, `create_test_channel_named`, `permissive_limiter` — 57 inline permissive-limiter sites replaced); all-`None` request literals collapsed via `..Default::default()` (one production `Default` derive added, on `GetRecentMessagesRequest`). Tests 723 → 705 (−19 pruned mock-only tests, +1 new `EnvGuard` panic test) |

**Legend:** ⬜ Pending | 🔄 In Progress | ✅ Complete | ❌ Blocked

**Overall Progress:** 39/39 phases complete
